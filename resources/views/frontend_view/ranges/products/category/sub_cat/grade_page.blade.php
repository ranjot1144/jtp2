                                        ...
                                        foreach($grade_data as $zindex => $data) {
                                          ...
                                          echo '<tr class="parent" id="row'.$data->grade_range.'" title="Click to expand/collapse" style="cursor: pointer;">
                                            ...
                                            if($filter_val==$grade_data[$zindex]->grade_range) {
                                              ...
                                              echo '<tr class="child-row1 child-row'.$grade_data[$zindex]->grade_range.'" style="'.$display_item.';background-color:#ebecec;">
                                                      <th> </td>  
                                                      <th colspan="4">Grade '.$data->grade_range.' </td>  
                                                      <th colspan="2"> Sizes <br/>(Diameter/mm) </td> 
                                                      <th> Product Code</td>  
                                                    </tr>

                                                    <tr class="child-row1 child-row'.$grade_data[$zindex]->grade_range.'" style="'.$display_item.'">
                                                      <td> </td>  
                                                      <td colspan="4" rowspan='.$cnt.' style="text-align:left; vertical-align: top; width:50%; background:#fff;">'.$grade_desc.'
                                                    </tr>';
                                                if($grade_code->isNotEmpty()) {
                                                  if($data->grade_id==$grade_code[$zindex]->grade_desc_id) {
                                                    foreach($grade_code as $index => $gc) { 
                                                      echo '<tr class="child-row1 child-row'.$grade_data[$zindex]->grade_range.'" style="'.$display_item.'">
                                                            <td> </td>
                                                            
                                                            <td colspan="2">'.$gc->gc_size.'</td>
